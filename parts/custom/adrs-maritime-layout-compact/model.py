...
def din_rail(length: float, t: float = 1.0, width: float = 35.0,
             height: float = 7.5, base_width: float = 27.0) -> cq.Workplane:
    """
    TS35 top-hat rail (EN 60715, 35 x 7.5), running along X and centred on it.

    Stated directly in ENCLOSURE axes like `plate_box`: the web lies on the
    subpanel at Z = 0, the two upstands rise to Z = height, and the two lips
    turn outward at the top to the full 35 mm across. Y is the rail width, so
    a module clipped to it straddles Y and consumes `its own thickness` of X.

    Modelled as the union of five prisms rather than a swept 1 mm strip: the
    section is what a device clip and a neighbouring bracket actually see, and
    five boxes cannot fail a fillet the way a swept profile can.
    """
    hw, hb = width / 2.0, base_width / 2.0
    lip = hw - hb + t                       # 5.0 mm of lip on a 35/27 rail
    parts = [
        (base_width, t,      0.0,              0.0),          # web, flat on the panel
        (t,          height, -hb + t / 2,      0.0),          # -Y upstand
        (t,          height, hb - t / 2,       0.0),          # +Y upstand
        (lip,        t,      -(hw + hb - t) / 2, height - t),  # -Y lip
        (lip,        t,      (hw + hb - t) / 2,  height - t),  # +Y lip
    ]
    rail = None
    for dy, dz, yc, z0 in parts:
        b = (cq.Workplane("XY")
             .box(length, dy, dz, centered=(True, True, False))
             .translate((0.0, yc, z0)))
        rail = b if rail is None else rail.union(b)
    return rail


_KINDS = {
    "box": lambda p: plate_box(p["dx"], p["dy"], p["dz"], p.get("corner_r", 0.0)),
    "din_rail": lambda p: din_rail(p["length"], p.get("t", 1.0), p.get("width", 35.0),
                                   p.get("height", 7.5), p.get("base_width", 27.0)),
