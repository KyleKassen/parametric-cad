"""D1 direct B210 plate, millimetres; reproducible parametric CadQuery source.
...
Original device geometry is preserved; only four feet are suppressed in assembly.
"""
...
def housing_points(p: dict) -> list[tuple[float, float]]:
    return [
        (sx * p["housing_pitch_x"] / 2, sy * p["housing_pitch_y"] / 2)
        for sx in (-1, 1)
        for sy in (-1, 1)
    ]
...
def housing_screw(p: dict) -> cq.Shape:
    ...
    Catalog 1.7 mm head-height is retained in params; the nominal1.5mm cone
    is not a complete or conservative representation of the real head.
    The nominal cone seat and overall6mm length govern this reference envelope.
    """
...
def panel_stud(p: dict) -> cq.Shape:
    ...
    return (
        ...
        .fuse(cyl(p["screw_diameter"], p["stud_projection"]))
        .clean()
    )
...
def nut(p: dict) -> cq.Shape:
    ...
    return s.cut(cyl(p["screw_diameter"], p["nut_height_nominal"]))


def normalize(s: cq.Shape, p: dict) -> cq.Shape:
    return (
        s.rotate((0, 0, 0), (1, 0, 0), p["device_normalize_rotate_x_deg"])
        .rotate((0, 0, 0), (0, 0, 1), p["device_normalize_rotate_z_deg"])
        .translate(tuple(p["device_normalize_translate"]))
    )


def load_device(source: Path, p: dict, work: Path) -> list[cq.Shape]:
    digest = hashlib.sha256(source.read_bytes()).hexdigest()
    if digest != p["input_step_sha256"]:
        raise ValueError(
            "Device STEP changed: repeat component identity audit before using solid indices."
        )
    ...
    assert len(shapes) == 196
    for i in p["device_foot_indices_zero_based"]:
        bb = shapes[i].BoundingBox()
        assert abs(bb.zmin + 3.556) < 0.001 and abs(bb.xlen - 12.7) < 0.001
    return shapes
...
def components(p: dict, shapes: list[cq.Shape]) -> list[tuple[str, cq.Shape, str]]:
    t = p["plate_thickness"]
    items = [
        ("adapter_D1", create_part(p).val(), "plate"),
        ("FPE_PANEL_REFERENCE_ONLY", panel_reference(p), "panel"),
    ]
    for i, s in enumerate(shapes):
        if i not in p["device_foot_indices_zero_based"]:
            items.append((f"device_original_solid_{i:03d}", s.translate((0, 0, t)), "device"))
    for i, (x, y) in enumerate(housing_points(p), 1):
        items.append(
            (f"McMaster_91294A126_M3x6_{i}", housing_screw(p).translate((x, y, 0)), "screw")
        )
    for i, (x, y) in enumerate(stud_points(p), 1):
        items += [
...
def export_all(p: dict, source: Path, output: Path, work: Path) -> tuple:
    ...
    print("Importing original device and building D1...", flush=True)
    shapes = load_device(source, p, work)
    items = components(p, shapes)
    plate = items[0][1]
    cq.exporters.export(plate, str(work / "plate_D1.step"))
    cq.exporters.export(
        plate, str(work / "plate_D1_preview.stl"), tolerance=0.03, angularTolerance=0.1
    )
    export_step(items, work / "assembled_device_mount_D1.step")
    export_step(
        [i for i in items if i[2] in {"plate", "device", "screw"}], work / "device_adapter_D1.step"
    )
    export_step(
        [i for i in items if i[2] in {"plate", "screw", "stud", "washer", "nut"}],
        work / "mount_hardware_D1.step",
    )
    export_step(
        [i for i in items if i[2] in {"panel", "stud"}], work / "FPE_interface_REFERENCE_ONLY.step"
    )
    cq.exporters.export(
        cq.Compound.makeCompound([shapes[i] for i in p["device_foot_indices_zero_based"]]),
        str(work / "removed_feet_REFERENCE_ONLY.step"),
    )
    planar_files(p, work)
    metadata = {
        "revision": p["revision"],
        ...
        "source_device_solids": 196,
        "installed_device_solids": 192,
        ...
        "datum": "Plate underside Z0; radio pan Z3.5; original input preserved.",
    }
...
def main() -> None:
    ...
    if args.work_dir:
        ...
        with tempfile.TemporaryDirectory(prefix="b210_direct_") as td:
