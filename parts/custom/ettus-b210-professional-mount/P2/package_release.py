...
def package(staging):
    ...
    for fname in ['verification_report.json','hardware_verification.json','additional_verification.json']:
    ...
    reader=PdfReader(target/'B210_mount_shop_pack_P2.pdf')
    assert len(reader.pages)==6
    all_text='\n'.join(page.extract_text() or '' for page in reader.pages)
    for required in ['M01','M02','M03','M04','M05','PRELIMINARY','0.20-0.30','M5']:
    ...
    archive=HERE/'B210_Mount_P2_Deliverables.zip'
    ...
    manifest={'revision':'P2','status':'Prototype design only; physical validation outstanding',
    ...
    with zipfile.ZipFile(archive,'w',zipfile.ZIP_DEFLATED,compresslevel=6) as z:
        for f in files:z.write(f,'B210_Mount_P2/'+f.relative_to(HERE).as_posix())
    with zipfile.ZipFile(archive) as z:assert z.testzip() is None
    print(json.dumps({'archive':str(archive),'bytes':archive.stat().st_size,'files':len(files),'geometry_checks':34,'hardware_checks':18,'additional_checks':3,'sections':sections,'manufacturing':dxf_report},indent=2))
