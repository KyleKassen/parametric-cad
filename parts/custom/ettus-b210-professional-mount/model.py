"""B210 P1: reproducible, millimetre-based CadQuery source.
...
def create_base(p):
    ...
    for x in (-p["retainer_bolt_x"], p["retainer_bolt_x"]):
        for y in stations(p):
            s = s.cut(cylinder(p["tap_minor_representation"], t+2, x, y, -1))
...
def flat_items(p, source=None, panel_hardware=True):
    ...
    zbar = p["base_thickness"]+p["spacer_length"]
    for j,y in enumerate(stations(p)):
        ...
        for i,x in enumerate((-p["retainer_bolt_x"], p["retainer_bolt_x"])):
            tag=f"{j+1}_{i+1}"
            items += [(f"B210_M03_spacer_{tag}", create_spacer(p).translate((x,y,p["base_thickness"])), "spacer"),
                      (f"M5_washer_{tag}", washer().translate((x,y,zbar+p["bar_thickness"])), "hardware"),
                      (f"M5x50_retainer_{tag}", cap_screw(p["retainer_bolt_length"]).translate((x,y,zbar+p["bar_thickness"]+1)), "hardware")]
    ...
    if panel_hardware:
        for i,x in enumerate((-p["panel_bolt_x"], p["panel_bolt_x"])):
            for j,y in enumerate([p["center_y"]+s*p["panel_bolt_y_pitch"]/2 for s in (-1,1)]):
                tag=f"{i+1}_{j+1}"
                items += [(f"flat_panel_M5x30_{tag}", cap_screw(30).translate((x,y,13)),"hardware"),
                          (f"flat_panel_upper_washer_{tag}", washer().translate((x,y,12)),"hardware"),
...
def upright_items(p, items=None):
    ...
    for i,y in enumerate(va.PARAMS["bracket_y_centers"]):
        for j,z in enumerate(va.PARAMS["web_hole_z"]):
            tag=f"{i+1}_{j+1}"
            out += [(f"upright_web_M5x30_{tag}", cap_screw(30).rotate((0,0,0),(0,1,0),-90).translate((-9,y,z)),"hardware"),
                    (f"upright_web_rear_washer_{tag}",washer().rotate((0,0,0),(0,1,0),90).translate((-9,y,z)),"hardware"),
                    (f"upright_web_front_washer_{tag}",washer().rotate((0,0,0),(0,1,0),90).translate((12,y,z)),"hardware"),
                    (f"upright_web_locknut_{tag}",nut().rotate((0,0,0),(0,1,0),90).translate((13,y,z)),"hardware")]
        for j,x in enumerate(va.PARAMS["foot_hole_x"]):
            tag=f"{i+1}_{j+1}"
            out += [(f"upright_foot_M5x25_{tag}",cap_screw(25).translate((x,y,9)),"hardware"),
                    (f"upright_foot_upper_washer_{tag}",washer().translate((x,y,8)),"hardware"),
...
def export_all(p, source, out):
    ...
    for name,shape in parts.items():
        ...
        cq.exporters.export(shape, str(out/f"{name}_P1.step"))
    items=flat_items(p,source)
    assembly(items,"B210_FLAT_P1").save(str(out/"B210_flat_assembly_P1.step"))
    ...
    assembly(exploded,"B210_SERVICE_P1").save(str(out/"B210_exploded_P1.step"))
    try:
        import vertical_adapter as va
        upright=upright_items(p,items)
        assembly(upright,"B210_UPRIGHT_P1").save(str(out/"B210_upright_assembly_P1.step"))
    except ImportError:
        upright=[]
...
def main():
    ...
    if args.render:
        ...
        for axis,station in (("Y",p["center_y"]-35),("X",0)):
            clipped=[]
            for _,s,k in items:
                bb=s.BoundingBox()
    ...
    print(f"Exported P1 to {args.out}")
