...
def run(out):
    ...
    for n in ["B210_M01_base","B210_M02_retainer","B210_M03_spacer","B210_M04_film"]:
        path=out/f"{n}_P1.step"; s=cq.importers.importStep(str(path)).val(); bb=s.BoundingBox()
    ...
    corridors={"RF_plug_and_bend":m.block(124,60,42,0,112.3115,12.5),
               "USB_DC_ref_plug_and_bend":m.block(124,70,42,0,-111.2055,12.5),
               "Kslot_front":m.block(20,17,13,71.1745,68.7295,18.91),
               "Kslot_rear":m.block(20,17,13,-71.1735,-62.8645,18.91)}
    ...
    for x in [-p['panel_bolt_x'],p['panel_bolt_x']]:
        for sy in [-1,1]:
            tool=m.cylinder(14,90,x,p['center_y']+sy*70,17)
    ...
    zpan=p['device_pan_z']; cy=p['center_y']; up=.4+.2+.151+.022+.065; down=-.2
    tilt=math.degrees(math.atan((up-down)/70)); zm=(up+down)/2
    witnesses=[]
    for sign in [-1,1]:
        pose=dev.rotate((0,cy,zpan),(1,cy,zpan),sign*tilt).translate((0,sign*3,zm))
        end=m.block(80,18,p['end_stop_top']-6,0, p['pocket_ymax']+9 if sign>0 else p['pocket_ymin']-9,6)
    ...
    check("retainer_screw_stack",abs(50-(8+1+30)-11)<1e-8,
          {"under_head_length":50,"grip":39,"nominal_engagement":11,"nominal_tip_above_base":1},
          "shop verify >=8 mm full threads and >=0.5 mm tip recess after actual tolerances")
    for name in ('B210_flat_assembly_P1','B210_upright_assembly_P1'):
