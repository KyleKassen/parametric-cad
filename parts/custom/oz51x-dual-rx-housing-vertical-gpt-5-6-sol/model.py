"""GPT-5.6 Sol production refinement of the OZ51x dual-RX vertical housing."""
...
if __name__ == "__main__":
    ...
    print(
        f"  Outer envelope: {L['envelope_width']:.1f} (W) x "
        f"{L['envelope_depth']:.1f} (D) x "
    ...
    export_part(
        create_base(params),
        name=f"{PART_DIR.name}_base",
        version=params.get("version", "v1"),
        formats=fmts,
    )
    export_part(
        create_lid(params),
        name=f"{PART_DIR.name}_service-cover",
        version=params.get("version", "v1"),
        formats=fmts,
    )
