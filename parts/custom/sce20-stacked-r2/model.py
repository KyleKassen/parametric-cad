...
def parameters():
    ...
    p["placements"] = {k: dict(translation_mm=v, rotation_z_deg=r) for k, (v, r) in placements.items()}
    ...
    for key in ["meanwell", "bedrock", "b210", "oz", "carrier"]:
        pattern = (interface[key]["backpanel_studs"]["local_xy_mm"] if key in interface
                   else small["oz"]["mounting_slots_xy"] if key == "oz"
        ...
        for i, (x, y) in enumerate(pattern, 1):
            ...
            p["hardware"].append(dict(
                id=key.upper()+"_"+str(i), component=key,
                x=math.floor((xx+1e-8)*1000)/1000,
                y=math.floor((yy+1e-8)*1000)/1000,
                catalog_code="WGO30" if raised else "WGU30",
                length=20 if raised else 12, side="front", depth_offset=0,
                seat_height={"meanwell":6,"bedrock":6,"oz":3,"b210":54,"carrier":74}[key],
                extension_length=30 if key=="b210" else 50 if key=="carrier" else 0,
                extension_part="971300321" if key=="b210" else "971500321" if key=="carrier" else None,
            ))
    p["clear_region"] = dict(x=[0, 215.9], y=[250, 431.8],
        note="Free of component and declared cable/air envelopes; retain existing factory hardware/tool access.")
    return p
...
def raised_hardware(p):
    ...
    for h in p["hardware"]:
        ...
        ext=(cq.Workplane("XY").polygon(6,5.5/math.cos(math.pi/6)).extrude(height).val()
             .translate((x,y,20)))
        ext=ext.fuse(cyl(3,6,x,y,14)).cut(cyl(3,7,x,y,20+height-7))
        top=h["seat_height"]
        washer=cyl(7,.55,x,y,top).cut(cyl(3.2,.75,x,y,top-.1))
        screw=cyl(3,10,x,y,top+.55-10).fuse(cyl(5.5,3,x,y,top+.55))
        out.extend([(h["id"]+"_WGO30_L20",base,"hardware"),
...
def router_hardware():
    ...
    for i,(dx,dy) in enumerate(itertools.product([-80.9,80.9],[-41.35,41.35]),1):
        ...
        out.extend([(f"PEPLINK_{i}_WGO40_L6",base,"hardware"),
...
def generate():
    ...
    check("Backpanel exported/reopened valid single solid",panel.isValid() and len(panel.Solids())==1)
    b=bounds(panel)
    check("431.8 square x6 dimensions",all(abs(b[a][1]-b[a][0]-n)<1e-5 for a,n in zip("xyz",[431.8,431.8,6])),b)
    for h in p["mounting_holes"]:
        check(h["id"]+" factory bore clear",abs(panel.intersect(cyl(h["diameter"]-.001,6.2,h["x"],h["y"],-6.1)).Volume())<1e-5)
    for h in p["hardware"]:
        check(h["id"]+" native cavity envelope",abs(panel.intersect(cyl(12.099,2.299,h["x"],h["y"],-2.299)).Volume())<1e-5)
    webs=[]
    for a,b in itertools.combinations(p["hardware"]+p["mounting_holes"],2):
        ...
        webs.append((math.hypot(a["x"]-b["x"],a["y"]-b["y"])-(da+db)/2,a["id"],b["id"]))
    check("All machining webs at least0.5mm",min(webs)[0]>=.5,min(webs))
    check("All native cavity edge webs at least3mm",min(min(h["x"],h["y"],431.8-h["x"],431.8-h["y"])-6.05 for h in p["hardware"])>=3)
    ...
    for key,path in old.SOURCES.items():
        components[key]=old.transformed(cq.importers.importStep(str(path)).val(),p["placements"][key])
    ...
    colors={**old.COLORS,"carrier":(.72,.64,.43)}
    old.COLORS.update(colors)
    for key,s in components.items():
        b=bounds(s)
        check(key+" contained in panel/door envelope",b["x"][0]>=-1e-4 and b["x"][1]<=431.8001 and b["y"][0]>=-1e-4 and b["y"][1]<=431.8001 and b["z"][0]>=-1e-4 and b["z"][1]<237.5352,b)
    for (ka,a),(kb,b) in itertools.combinations(components.items(),2):
        gap=a.distance(b)
        check(ka+" / "+kb+" physical clearance",gap>.1,gap)
    ...
    for n,h,k in hw:
        ...
        for key,s in components.items():
            ...
            d=h.distance(s)
            if d<.1:
                if owner=="carrier" and key=="bedrock" and n.endswith("_WGO30_L20"):
                    ...
                    check(n+" recessed flange flush beneath Bedrock edge",abs(v)<1e-5,v)
                else:
                    check(n+" unrelated "+key+" interference",False,d)
    check("Bedrock remains at direct original thermal seating plane",p["placements"]["bedrock"]["translation_mm"][2]==0)
    check("Bedrock fin-to-carrier underside gap13mm",abs(components["carrier"].BoundingBox().zmin-components["bedrock"].BoundingBox().zmax-13)<1e-4)
    check("OZ-to-B210 underside gap17.28mm",abs(components["b210"].BoundingBox().zmin-components["oz"].BoundingBox().zmax-17.28)<1e-4)
    check("Upper M3x10 screw insertion5.45mm within7mm blind thread",0<10-4-.55<7)
    check("Extension male6mm within native10mm thread",6<10)
    items=[("BACKPANEL_R2",panel,"panel")]+hw+[(k.upper()+"_INSTALLED",s,k) for k,s in components.items()]
    old.export(items,out/"Installed_R2.step")
    saved=cq.importers.importStep(str(out/"Installed_R2.step")).val()
    check("Assembly STEP reopened solid count",len(saved.Solids())==sum(len(s.Solids()) for _,s,_ in items),len(saved.Solids()))
    enclosure=cq.Shape.importBrep(str(OLD/"references/enclosure/source_enclosure.brep"))
    enc=[(f"SAGINAW_{i}",s.translate((0,0,2.825)) if i in [61,62,63,64] else s,"enclosure") for i,s in enumerate(enclosure.Solids()) if i!=60]
    old.export(enc+[(n,s.translate((-215.9,-215.9,-106.395)),k) for n,s,k in items],out/"Enclosure_Installed_R2.step")
    report=dict(status="PASS" if all(c["pass_check"] for c in checks) else "FAIL",checks=checks,
                maximum_height_mm=109.3,door_margin_mm=128.2352,
                scope="Nominal CAD integration; simplified threads; inherited invalid Peplink connector solids; no thermal/load qualification.")
    (out/"cad_verification.json").write_text(json.dumps(report,indent=2),encoding="utf-8")
    render=module(ROOT/"parts/custom/solidrun-bedrock-thermal-stud-mount/render_support.py","sce20_r2_renderer")
    render.render_scene([(s,colors[k],1) for _,s,k in items],HERE/"renders","Installed_R2",views=("top","iso"),size=1500,axes=False)
    ...
    for n,s,k in items:
        ...
        if k=="hardware" and (n.startswith("PEPLINK_") or (n.startswith(("CARRIER_","B210_")) and n.endswith(("_washer","_M3x10")))):
            lift=90
        exploded.append((s.translate((0,0,lift)),colors[k],1))
    render.render_scene(exploded,HERE/"renders","Exploded_R2",views=("iso",),size=1500,axes=False)
