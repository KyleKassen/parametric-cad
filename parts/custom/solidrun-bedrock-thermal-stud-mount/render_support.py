...
def section_cut(
    ...
    solids = shape.Solids()
    if len(solids) > 1:
        # The vendor assembly contains overlapping overlays/parts. A compound
        # Boolean may retain entire members; section each solid independently.
        kept = []
        for solid in solids:
            sb = solid.BoundingBox()
            smin = (sb.xmin, sb.ymin, sb.zmin)[i]
            smax = (sb.xmax, sb.ymax, sb.zmax)[i]
            if smin >= station - 1e-6:
                continue
            cut = solid if smax <= station + 1e-6 else solid.cut(cutter)
            if cut.Volume() > 1e-9:
                kept.extend(cut.Solids())
        result = cq.Compound.makeCompound(kept)
    else:
        result = shape.cut(cutter)
    rb = result.BoundingBox()
    if (rb.xmax, rb.ymax, rb.zmax)[i] > station + 1e-4:
        raise ValueError('Section left material beyond the section plane')
