...
def main(part_dir: Path = PART_DIR) -> int:
    ...
    modules = [place_module(VENDOR / bay["step"], cx, L["plate_bottom_z"])
    ...
    checks += [("lid", lid)]
    checks += [(f"SC adapter {b['label']}", a)
               for b, a in zip(L["bays"], adapters)]
    checks += [(f"SC connector {b['label']}", c)
               for b, c in zip(L["bays"], connectors)]
    ...
    module_bits = [(mod.val(), ORANGE if b.get("mirror_x") else BLUE, 1.0)
    ...
    written = render_scene(
        ...
        out, "fit_open", views=("iso", "front", "top", "back"),
    )
    written += render_scene(
        [(base.val(), SHELL, 0.25), (lid.val(), SHELL, 0.25)]
        + module_bits + fiber_bits,
        out, "fit_closed", views=("iso", "back"),
    )
    for p in written:
