...
## Validation and visual review — final candidate
...
base 105,883.308 mm³ and cover 28,098.771 mm³, one valid solid each. Total material
volume is 133,982.078 mm³ against 147,913.679 mm³ in the original v2 STEP: **9.42%
less material**. Geometry checks pass; no print time or measured mass is claimed.

`fit_check.py` passes **40/40** checks on the exported solids. Seated overlap,
...
release travel all pass. Each SC adapter has a 0.010 mm³ reference-model overlap,
...
`20260908-120539-5fbb58`) scores **49.8/100, band D, role enclosure**, below the
unchanged hard bar of 70. Role is enclosure because this assembly houses electronics
and all exterior faces are product surfaces. `status=ok`, all metric populations
measured, no metric errors/absent defects, no role error and **config_delta=0.0**.
Both floors pass: body edge-break term **64.2 against 10**, margin 54.2;
sharp-edge score **52.37 against 25**, margin 27.37. Composite edge coverage is
...
- **High: symmetry, 0.0.** The measured best-plane wording saturates all planes
  ...
  changed that metric alone from 0 to 84. The final report still names 80.6%
  Y difference despite its Z volume difference being 2.86%. This limitation is
  recorded, not used to replace the score. Snap-vs-fixed-hook asymmetry is intentional.
...
and `baseline_review.json` to original v2. Do not substitute either number for
the final 49.8 score. The original has an unmeasured feature population, so its
...
| 17. Score record | 49.8/D and all retained high/medium findings disclosed. |
